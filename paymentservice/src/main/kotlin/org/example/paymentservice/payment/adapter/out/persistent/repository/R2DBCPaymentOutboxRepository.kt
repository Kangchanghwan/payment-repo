...
class R2DBCPaymentOutboxRepository(
    ...
    override fun getPendingPaymentOutboxes(): Flux<PaymentEventMessage> {
        return databaseClient.sql(SELECT_PENDING_PAYMENT_OUTBOX_QUERY)
            .bind("createAt", LocalDateTime.now().format(MySQLDateTimeFormatter))
            .fetch()
            .all()
            .map {
                PaymentEventMessage(
    ...
    private fun createPaymentMessage(command: PaymentStatusUpdateCommand): PaymentEventMessage {
        return PaymentEventMessage(
            ...
            metadata = mapOf(
                "partitionKey" to partitionKeyUtil.createPartitionKey(partitionKeyUtil.hashCode())
            )
        )
    }
    ...
    companion object {
        ...
        val SELECT_PENDING_PAYMENT_OUTBOX_QUERY = """
            ...
            AND created_at <= :createAt - INTERVAL 1 MINUTE 
