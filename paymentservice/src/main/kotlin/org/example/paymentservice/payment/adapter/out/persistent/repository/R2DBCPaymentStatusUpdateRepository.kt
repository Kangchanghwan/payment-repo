...
import org.example.paymentservice.payment.domain.PaymentEventPublisher
...
class R2DBCPaymentStatusUpdateRepository(
    ...
    private fun insertPaymentHistory(
        paymentOrderToStatus: List<Pair<Long, String>>,
        status: PaymentStatus,
        reason: String
    ): Mono<Long> {
        if(paymentOrderToStatus.isEmpty()) return Mono.empty()

        val valueClause = paymentOrderToStatus.joinToString(", ") {
            "( ${it.first}, '${it.second}', '${status}', '${reason}' )"
        }
        return databaseClient.sql(INSERT_PAYMENT_HISTORY_QUERY(valueClause))
            .fetch()
            .rowsUpdated()
    }
    ...
    private fun updatePaymentStatusToSuccess(command: PaymentStatusUpdateCommand): Mono<Boolean> {
        return selectPaymentOrderStatus(command.orderId)
            ...
            .flatMap { updatePaymentEventExtraDetail(command) }
    ...
    private fun updatePaymentEventExtraDetail(command: PaymentStatusUpdateCommand): Mono<Long> {
        return databaseClient.sql(UPDATE_PAYMENT_EVENT_EXTRA_DETAILS_QUERY)
            .bind("orderName", command.extraDetails!!.orderName)
            .bind("method", command.extraDetails.method)
    ...
    private fun incrementPaymentOrderFailedCount(command: PaymentStatusUpdateCommand): Mono<Long> {
        return databaseClient.sql(INCREMENT_PAYMENT_ORDER_FILED_COUNT_QUERY)
            .bind("orderId", command.orderId)
            .fetch()
            .rowsUpdated()
    }
    ...
    companion object {
        ...
        val UPDATE_PAYMENT_EVENT_EXTRA_DETAILS_QUERY = """
            ...
            WHERE order_id = :orderId
        """.trimIndent()
        val INCREMENT_PAYMENT_ORDER_FILED_COUNT_QUERY = """
            UPDATE payment_orders
