...
class R2DBCPaymentDatabaseHelper(
    ...
    private fun deletePaymentEvents() = databaseClient.sql(DELETE_PAYMENT_EVENT_QUERY)
        .fetch().rowsUpdated()

    private fun deleteOutboxes() = databaseClient.sql(DELETE_OUTBOX_QUERY).fetch().rowsUpdated()
